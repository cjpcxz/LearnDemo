...
class BSTIterator {
        ...
        var begin: Int = -1
    init(_ root: TreeNode?) {
        arr = BSTIterator.buildArr(root:root)
    }

    private static func buildArr(root:TreeNode?) -> [Int] {
        guard let root = root else {
            return []
        }
        return  buildArr(root:root.left) + [root.val] + buildArr(root:root.right)
    }
    
    func next() -> Int {
        begin += 1
        guard begin < arr.count else {
            return -1
        }
        return arr[begin]
    }
