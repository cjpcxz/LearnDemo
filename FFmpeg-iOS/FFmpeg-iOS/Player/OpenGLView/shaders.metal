...
samplingShader(RasterizerData input [[stage_in]], // stage_in表示这个数据来自光栅化。（光栅化是顶点处理之后的步骤，业务层无法修改）
               ...
               texture2d<float> textureU [[ texture(LYFragmentTextureIndexTextureU) ]], // texture表明是纹理数据，LYFragmentTextureIndexTextureU是索引
               texture2d<float> textureV [[ texture(LYFragmentTextureIndexTextureV) ]], // texture表明是纹理数据，LYFragmentTextureIndexTextureV是索引
