...
if ! [ -f $LAME_FILE ];then
	...
	curl -o $LAME_FILE http://ncu.dl.sourceforge.net/project/lame/lame/3.100/lame-3.100.tar.gz
	if [ $? != 0 ];then
...
then
	...
	do
		...
		then
		    ...
		    if [ "$ARCH" = "x86_64" ]
		    then
		    	SIMULATOR="-mios-simulator-version-min=$IOS_VERSION_MIN"
